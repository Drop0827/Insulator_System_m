...
class CBAM(nn.Module):
    def __init__(self, c1, kernel_size=7):  # c1是输入通道数
...
# --- 毕业设计改进：GSConv (来自 Slim-neck 论文，针对缺陷检测优化) ---
class GSConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()
        c_ = c2 // 2
        # 显式指定 act=act，跳过中间的 d (dilation) 参数
    ...
    def forward(self, x):
        ...
        b, n, h, w = y.data.size()
        b_n = b * n // 2
        y = y.reshape(b_n, 2, h, w).permute(1, 0, 2, 3).reshape(b, n, h, w)
        return y


# --- 修正后的 DySample (针对小目标检测优化版) ---
class DySample(nn.Module):
    def __init__(self, in_channels, scale=2):
        super().__init__()
        self.scale = scale
        # 核心修改：强制输出通道为 2 * scale^2 (即 2 * 4 = 8)
        # 这样经过 pixel_shuffle 之后，通道数会变为 2，完美匹配 grid
        self.offset_conv = nn.Conv2d(in_channels, 2 * (scale ** 2), 1)

    def forward(self, x):
        b, c, h, w = x.shape
        hh, ww = h * self.scale, w * self.scale

        # 1. 生成偏移量并转换为 2 通道
        offset = self.offset_conv(x)
        offset = F.pixel_shuffle(offset, self.scale)  # (B, 2, hh, ww)

        # 2. 生成基础网格
        grid = self._get_grid(b, hh, ww, x.device).to(x.dtype)  # (B, hh, ww, 2)

        # 3. 转换偏移量维度并相加
        offset = offset.permute(0, 2, 3, 1)  # (B, hh, ww, 2)
        grid = grid + offset
    ...
    def _get_grid(self, b, h, w, device):
        ...
        grid = (grid + 0.5) / torch.tensor([w, h], device=device) * 2 - 1
        return grid.unsqueeze(0).repeat(b, 1, 1, 1)


class SPD(nn.Module):
    # 空间像素下采样 (Space-to-Depth)，专门用于保留细微特征
    def __init__(self, dimension=1):
        super().__init__()
        self.d = dimension

    def forward(self, x):
        # 将空间维度转移到通道维度，不丢失任何像素
